...
项目按标准 src-layout 组织：所有 Python 包在 `src/javlibraryscrapy/` 下；`scripts/` 仅放 PowerShell 运维脚本；`tests/` 复数（pytest 习惯）按 `unit/` 和 `integration/` 分。
...
# 等价于：uv run python -m javlibraryscrapy.cli.export_mostwanted --source output/javlibrary_movies.json --library-root "$MOSTWANTED_LIBRARY_ROOT"
uv run python -m javlibraryscrapy.cli.export_mostwanted --library-root "Z:\\JAV\\MostWanted" --overwrite  # 强制覆盖已存在
uv run python -m javlibraryscrapy.cli.export_mostwanted --dry-run  # 只打印计划，不写文件
uv run python -m javlibraryscrapy.cli.export_mostwanted --limit 5  # 只处理前 5 部（调试用）
uv run python -m javlibraryscrapy.cli.export_mostwanted --skip-javbus  # 只下 poster.jpg，跳过 JAVBus 抓取
...
uv run python -m javlibraryscrapy.cli.gallery --library-root "Z:\\JAV"
# 等价的 console_script（pyproject [project.scripts] 暴露）：
javlibraryscrapy-gallery --help
...
uv run python -m javlibraryscrapy.cli.workflow <download_path> <intermediate_path> <output_path> [--min-size 500] [--preview]
...
> `tests/` 目录下是手动调试脚本 + pytest 测试，按 `unit/` / `integration/` 分目录。

## 架构

### 爬虫（顶层入口）

- **`src/javlibraryscrapy/scraping/javbus.py`** — `JavbusSpider`：扫描视频目录，提取车牌，通过 `AsyncDynamicSession` 抓取 JAVBus 视频详情页，下载封面，在 `<root>/<CARID> <title>/` 下生成 NFO + poster/fanart。
...
- **`gallery.py`** — CLI 入口（argparse + uvicorn.run）。等价于 `python -m javlibraryscrapy.cli.gallery` 或 `javlibraryscrapy-gallery`（[project.scripts] 暴露）。

### `src/javlibraryscrapy/server/`

- FastAPI + uvicorn 的重构版本。包布局：
...
> - `Move-VideoFiles.ps1` / `Rename-FilesByAtSymbol.ps1` —— 对应早期的 `src/javlibraryscrapy/cli/move_videos.py` / `src/javlibraryscrapy/cli/rename_at_symbol.py`，保留以备旧流程使用。

### `src/javlibraryscrapy/cli/workflow.py`（权威的端到端流水线）
...
- `src/javlibraryscrapy/library/scanner.py` — 独立模块，提供：
  ...
  - CLI：`uv run python -m javlibraryscrapy.library.scanner --root Z:\JAV`
